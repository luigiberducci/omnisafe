...
class RobustGaussianLearningActor(GaussianActor):
    """Implementation of GaussianLearningActor.

    RobustGaussianLearningActor is a Gaussian actor with a learnable standard deviation and
    perturbed distribution used to compute the log probability.
    It is used in the on-policy algorithm ``RPO``.
    ...
    def log_prob(self, act: torch.Tensor) -> torch.Tensor:
        """
        Compute the log probability of the action given the current distribution uniformly perturbed.
        This is the new part introduced by RPO compared to the usual Gaussian actor.
    ...
    def perturbed_log_prob(self, act: torch.Tensor, rpo_alpha: float) -> torch.Tensor:
        """
        Compute the log probability of the action given the current distribution uniformly perturbed.
        This is the new part introduced by RPO compared to the usual Gaussian actor.
        ...
        action_mean = self._current_dist.mean
        action_std = self._current_dist.stddev
        z = torch.Tensor(action_mean.shape).uniform_(-rpo_alpha, rpo_alpha).to(action_mean.device)
        # perturb the action mean
        perturbed_action_mean = action_mean + z
        # compute the new distribution
        new_distr = Normal(perturbed_action_mean, action_std)

        return new_distr.log_prob(act).sum(axis=-1)
